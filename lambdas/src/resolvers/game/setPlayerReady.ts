...
import { updateGameSetPlayerReady } from "@/lib/controller/dynamo/query/game";
...
export const resolver: AppHandler<SetPlayerReadyMutationVariables, Game> = async (event) => {
  ...
  const updatedGameEntity = await updateGameSetPlayerReady(client, {
    gameId: gameId,
